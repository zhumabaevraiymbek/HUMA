...
async function runAnalysis(textToAnalyze: string, lang: string, fileName: string) {
  const truncated = textToAnalyze.length > 40000;
  const text = truncated ? textToAnalyze.slice(0, 40000) : textToAnalyze;
  const paragraphs = splitIntoParagraphs(text);

  if (paragraphs.length === 0) {
    return NextResponse.json({ error: 'Could not extract paragraphs from text.' }, { status: 422 });
  }

  const langInstruction = LANG_INSTRUCTIONS[lang as keyof typeof LANG_INSTRUCTIONS] || LANG_INSTRUCTIONS.en;

  const prompt = `You are an expert forensic linguist specializing in detecting AI-generated text. Analyze the text below and determine what percentage was likely generated by an AI language model.
  ...
  try {
    ...
    if (plainText) {
      if (plainText.trim().length < 100) {
        return NextResponse.json({ error: 'Text is too short (minimum 100 characters).' }, { status: 422 });
      }
      return await runAnalysis(plainText, lang, 'plain-text');
    }

    // ── File mode ────────────────────────────────────────────────
    const file = formData.get('file') as File | null;
    if (!file) return NextResponse.json({ error: 'No file or text provided' }, { status: 400 });
    ...
    return await runAnalysis(rawText, lang, file.name);
  } catch (error: unknown) {
